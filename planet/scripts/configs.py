...
from planet import BATCHSIZE, CHUNK_LEN, NUM_SEED, H_SIZE, S_SIZE,NUM_GPU
...
def _data_processing(config, params):
  ...
  config.batch_shape = params.get('batch_shape', (BATCHSIZE, CHUNK_LEN,NUM_GPU))
...
def _active_collection(config, params):
  ...
  for task in config.tasks:
    for index, horizon in enumerate(params.get('collect_horizons', [12])):
      ...
      sim.steps_after = params.get('collect_every', 1)    # sim after 5000 steps
      sim.steps_every = params.get('collect_every', 1)    # sim every 5000 steps
